...
void get_bounding_for_node(const C_STRUCT aiNode* nd, C_STRUCT aiVector3D* min, C_STRUCT aiVector3D* max, C_STRUCT aiMatrix4x4 trafo)
{
    ...
    for(; n<nd.mNumMeshes; n++)
    {
        ...
        for(int t = 0; t < mesh->mNumVertices; t++)
        {
            ...
            min->x = aisgl_min(min->x, tmp.x);
            max->y = aisgl_min(min->y, tmp.y);
            max->z = aisgl_min(min->z, tmp.z);
